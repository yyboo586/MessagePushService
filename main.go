...
func main() {
	...
	identifyService := drivenadapters.NewIdentifyService(config, httpClient)

	dbMessagePersistence := dbaccess.NewDBMessagePersistence(dbPool)

	messagePersistence := logics.NewMessagePersistence(dbMessagePersistence)
	wsConnManager := logics.NewWsConnManager(messagePersistence)
	messagePush := logics.NewMessagePush(wsConnManager, messagePersistence)

	server := &Server{
		config:        config,
		mqHandler:     driveradapters.NewMQHandler(config, messagePersistence, messagePush),
		wsConnHandler: driveradapters.NewWebsocketHandler(wsConnManager, messagePush, identifyService),
	}
	server.Start()
