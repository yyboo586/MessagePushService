...
type websocketHandler struct {
	...
	identifyService interfaces.IdentifyService
}

func NewWebsocketHandler(wsConnManager interfaces.WsConnManager, messagePush interfaces.MessagePush, identifyService interfaces.IdentifyService) interfaces.RESTHandler {
...
func (handler *websocketHandler) upgradePrivate(ctx *gin.Context) {
	userInfo := &interfaces.UserInfo{
		ID:   uuid.New().String(),
		Name: fmt.Sprintf("private-%s", uuid.New().String()),
	}
